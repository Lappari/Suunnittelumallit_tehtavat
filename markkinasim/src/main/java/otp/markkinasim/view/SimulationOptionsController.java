...
public class SimulationOptionsController {
    ...
    private void handleEditParty() {
        IParty selectedParty = partyTable.getSelectionModel().getSelectedItem();
    ...
    private void handleEditRawmaterial() {
        IProduct selectedRawmaterial = rawmaterialTable.getSelectionModel().getSelectedItem();
	...
	private boolean rawmaterialUsed(IProduct rawmaterial) {
		for(IProduct i:view.getAllProductData()) {
			if(i.getProductNeededId()==rawmaterial.getId()) {
				return true;
			}
		}
		for(IParty i: view.getPartyData()) {
			if(i.getProductToProduce()==rawmaterial.getId()) {
				return true;
			}
		}
	...
	private boolean productUsed(IProduct product) {
		for(IParty i:view.getPartyData()) {
			if(i.getProductToProduce()==product.getId()) {
				return true;
			}
		}
		return false;
    ...
    private void handleEditProduct() {
    	IProduct selectedProduct = productTable.getSelectionModel().getSelectedItem();
