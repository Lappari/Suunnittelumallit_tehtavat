...
public class LvlUpvisitor extends AVisitor {
	...
	public LvlUpvisitor(Controller c) {
		super(c);
	}
	...
	public void lvlUp(PokemonContext c, Charmander asd) {
		...
		if (c.getLvl() >= 16) {// kun pokemon saavuttaa levelin 16 tai yli se muuttaa muotoa
			controller.printConsole(c.getName()+" muuttaa muotoaan ja on nyt "+Charmaleon.getInstace().getClass().getSimpleName()+"!\n");
			asd.evolve(c, Charmaleon.getInstace());
		} else if (c.getLvl() != 1) {
			controller.printConsole(c.getName()+" sai levelin!\n");
		}
		c.setHp(c.getMaxHp());
	}
	...
	public void lvlUp(PokemonContext c, Charmaleon asd) {
		...
		if (c.getLvl() >= 36) {// kun pokemon saavuttaa levelin 36 tai yli se muuttaa muotoa
			controller.printConsole(c.getName()+" muuttaa muotoaan ja on nyt "+Charizard.getInstace().getClass().getSimpleName()+"!\n");
	...
	public void lvlUp(PokemonContext c, Bulbasaur asd) {
		...
		if (c.getLvl() >= 16) {// kun pokemon saavuttaa levelin 16 tai yli se muuttaa muotoa
			controller.printConsole(c.getName()+" muuttaa muotoaan ja on nyt "+Ivysaur.getInstace().getClass().getSimpleName()+"!\n");
	...
	public void lvlUp(PokemonContext c, Ivysaur sad) {
		...
		if (c.getLvl() >= 36) {// kun pokemon saavuttaa levelin 36 tai yli se muuttaa muotoa
			controller.printConsole(c.getName()+" muuttaa muotoaan ja on nyt "+Venusaur.getInstace().getClass().getSimpleName()+"!\n");
